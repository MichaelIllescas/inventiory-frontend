  ...
  const PurchasesPage = () => {
    ...
    const filterPurchasesByDate = (purchases, startDate, endDate) => {
      return purchases.filter(purchase => {
          ...
          return (!start || purchaseDateObj >= start) && (!end || purchaseDateObj <= end);
      });
  };
  ...
  const handleReset = () => {
    ...
    <div className="container card mt-5 pt-5 table-responsive table" data-aos="fade-left">
      ...
      <div className="d-flex gap-3 mb-3">
        <div className="mx-3 mt-3 d-flex">
          ...
          <input type="date" className="form-control me-3" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
          <input type="date" className="form-control " value={endDate} onChange={(e) => setEndDate(e.target.value)} />
        </div>
       
        <div className="align-self-end">
        <button className="btn btn-primary me-2" onClick={handleFilter}>Filtrar</button>
    ...
    <EditPurchaseModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} purchaseData={selectedPurchase} onSubmit={handleSave} />
    ...
    <Modal show={isConfirmModalOpen} onHide={() => setIsConfirmModalOpen(false)} centered>
      ...
      <Modal.Body>
        ¿Está seguro de que desea {selectedPurchase?.state === "ACTIVO" ? "Anular" : "Activar"} la compra <strong>{selectedPurchase?.code}</strong>?
      </Modal.Body>
      <Modal.Footer>
        <Button variant="secondary" onClick={() => setIsConfirmModalOpen(false)}>Cancelar</Button>
        <Button variant={selectedPurchase?.state === "ACTIVO" ? "danger" : "success"} onClick={handleToggleStatus}>
