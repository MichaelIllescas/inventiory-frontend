...
const RegisterFreeTrial = () => {
  const { registerUserFree, loading, error, setError } = useRegisterFromLandingPage();
  ...
  const validateForm = () => {
    ...
    if (!/^\d+$/.test(form.documentNumber))
      errors.documentNumber = "Debe ser un número válido";
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      const result = await registerUserFree(form);
      if (result) {
        setConfirm(true);
        setForm({
          name: "",
  ...
  return (
    <Container className="py-5">
      <h1 className="text-center mb-4">Comenzá tu prueba gratuita</h1>

     

      <div className="d-flex flex-column justify-content-center w-75 m-auto">
        <p className="text-start text-grey mb-4 ms-5">
          ✅Accedé gratuitamente y probá todas las funcionalidades del sistema
          con límite de hasta 10 registros por sección.
        </p>
        <p className="text-start text-grey mb-4 ms-5">
          ✅Si Inventiory te es de utilidad, podrás cambiarte al plan PRO para
      ...
      <Form onSubmit={handleSubmit} noValidate className="shadow p-4 rounded">
        ...
        <div className="w-100 mt-5 text-center">
          <Button variant="success" type="submit" className="w-auto" disabled={loading}>
            {loading ? "Registrando..." : "Registrarme gratis"}
          </Button>
        </div>
        {confirm && (
        <div className="alert alert-success text-center mt-3  ">
          ✅ ¡Registro exitoso! Serás redirigido al login en {redirectCountdown} segundos...
        </div>
      )}
